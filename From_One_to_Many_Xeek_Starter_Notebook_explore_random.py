...
if __name__ == '__main__':
    ...
    pool = mp.Pool(processes=43)

    # Declare results variable
    results = None

    # Generate random parameter combinations
    random_samples = random.sample(list(itertools.product(*model_parameters_range.values(), *hyperparameters_range.values())), num_samples)

    # Use the pool to run the function in parallel
    try:
        results = pool.map(run_model, random_samples)
        ...
        if results is not None:
            ...
            for result in results:
                total_loss, params = result
                log_data.append((params, total_loss))
            ...
            with open('src/log_data_randome.csv', 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(['Parameter Combination', 'Loss'])
                writer.writerows([(str(params), loss) for params, loss in log_data])


"""
    # random_samples = [(200, False, 3, 64, [16, 2], 64, [8, 1], 10, 0.001, 100, 1, 0.02, 0.03, 10)]
    # random_samples = [(100, False, 1, 32, [8, 2], 64, [4, 1], 10, 0.001, 200, 5, 0.02, 0.03, 10)]
    # print(random_samples[0])
    # results = run_model(random_samples[0])
